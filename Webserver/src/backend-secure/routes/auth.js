...
router.post('/login',
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.log('[LOGIN] user count:', users.length);
      ...
      if (!user.password_hash) {
        console.error('[LOGIN] user has no password_hash column value');
        return res.status(500).json({
          success: false,
          message: '계정 설정 오류입니다. 관리자에게 문의하세요.'
        });
      }
      ...
      if (!jwtSecret) {
        ...
        return res.status(500).json({
          success: false,
          message: '서버 토큰 설정 오류(JWT_SECRET 미설정)'
        });
      }
